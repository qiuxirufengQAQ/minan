...
if systemctl is-active --quiet minan-game1; then
    systemctl restart minan-game1
    echo -e "${GREEN}✅ 服务已重启${NC}"
else
    systemctl start minan-game1
...
if systemctl is-active --quiet minan-game1; then
    ...
    echo "  前端：http://server-ip:3001"
    echo "  后端：http://server-ip:8081/api"
    echo ""
    echo -e "${BLUE}🔍 快速测试:${NC}"
    ...
    echo "查看日志：sudo journalctl -u minan-game1 -f"
    exit 1
fi
