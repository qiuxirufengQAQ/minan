...
| **数据库** | `lianai_game2` ✅ | `lianai_game1` |
| **后端端口** | `8082` ✅ | `8081` |
| **前端端口** | `3002` ✅ | `3001` |
| **部署目录** | `/var/www/lianai2/` ✅ | `/var/www/lianai1/` |
| **Systemd 服务** | `lianai-game2` ✅ | `lianai-game1` |
| **日志** | `journalctl -u lianai-game2` ✅ | `journalctl -u lianai-game1` |
...
# 查看表
mysql -u root -proot lianai_game2 -e "SHOW TABLES;"

# 备份数据库
mysqldump -u root -proot lianai_game2 > backup_$(date +%Y%m%d_%H%M%S).sql
```
...
spring:
  datasource:
    url: jdbc:mysql://localhost:3306/${DB_NAME:lianai_game2}
```

**systemd 设置环境变量**：

```ini
# /etc/systemd/system/lianai-game2.service
[Service]
...
- 随便修改你的数据库 `lianai_game2`
- 随时重启你的服务 `lianai-game2`
- 测试任何功能，不会影响小爪
- 独立部署，独立调试
- 修改表结构、添加字段
...
- ❌ 不要操作小爪的数据库 `lianai_game1`
- ❌ 不要停止小爪的服务 `lianai-game1`
- ❌ 不要覆盖小爪的部署目录 `/var/www/lianai1/`
- ❌ 不要占用小爪的端口 `8081`, `3001`
...
```bash
# 1. 检查服务状态
sudo systemctl is-active lianai-game2
# 应该输出：active
...
# 3. 检查数据库连接
mysql -u root -proot lianai_game2 -e "SELECT 1"

# 4. 检查 jar 文件
ls -lh /var/www/lianai2/game-1.0.0.jar
...
```bash
# 1. 检查数据库是否存在
mysql -u root -proot -e "SHOW DATABASES LIKE 'lianai_game2'"

# 2. 重新初始化数据库
mysql -u root -proot lianai_game2 < database/schema.sql
mysql -u root -proot lianai_game2 < migrations/20260305_ai_dual_role_complete.sql

# 3. 检查环境变量
sudo systemctl show lianai-game2 | grep DB_NAME
# 应该输出：DB_NAME=lianai_game2
```
...
| **日志目录** | `/var/www/lianai2/logs/` |
| **上传文件** | `/var/www/lianai2/uploads/` |
| **PID 文件** | `/var/www/lianai2/app.pid` |

### 系统配置

| 文件 | 路径 | 说明 |
|------|------|------|
| **systemd 服务** | `/etc/systemd/system/lianai-game2.service` | 你的服务配置 |
