...
public class StatisticsService {
    ...
    private StatisticsResponse.UserStats getUserStats() {
        ...
        LambdaQueryWrapper<com.minan.game.model.User> activeQuery = new LambdaQueryWrapper<>();
        // 这里简化处理，实际应该根据最后登录时间或评估时间判断
        // 暂时将所有用户都视为活跃用户
        userStats.setActiveUsers(totalUsersCount.intValue());
        
        // 平均魅力值
        List<com.minan.game.model.User> allUsers = userMapper.selectList(null);
        if (!allUsers.isEmpty()) {
    ...
    private StatisticsResponse.LevelStats getLevelStats() {
        ...
        Long completedLevelsCount = userLevelMapper.selectCount(
            new LambdaQueryWrapper<com.minan.game.model.UserLevel>()
                .eq(com.minan.game.model.UserLevel::getIsCompleted, true)
        );
    ...
    private StatisticsResponse.EvaluationStats getEvaluationStats() {
        ...
        // 获取所有评估记录
        List<com.minan.game.model.Evaluation> allEvaluations = evaluationMapper.selectList(null);
