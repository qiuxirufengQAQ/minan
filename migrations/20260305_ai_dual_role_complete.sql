-- ======================================
-- Minan AI 双角色系统完整迁移脚本
-- 创建时间：2026-03-05 23:10
-- 说明：合并两个 AI 迁移脚本，避免重复
-- ======================================
...
SELECT '✅ scene (AI columns)' AS check_item, COUNT(*) AS count 
FROM information_schema.COLUMNS 
WHERE TABLE_SCHEMA = DATABASE() 
  AND TABLE_NAME = 'scene' 
  AND COLUMN_NAME IN ('max_conversation_rounds', 'ai_npc_enabled', 'ai_coach_enabled');
SELECT '✅ evaluation (AI columns)' AS check_item, COUNT(*) AS count 
FROM information_schema.COLUMNS 
WHERE TABLE_SCHEMA = DATABASE() 
...
SELECT '🎉 AI 双角色系统数据库迁移完成！' AS message;
